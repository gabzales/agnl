...
const initDB = async () => {
  ...
  const defaultSettings = {
    ...
    genspay: { apiKey: '', baseUrl: 'https://stenly.id/api/v1' },
...
// 📖 Dokumentasi Integrasi: https://stenly.id/docs
// Base URL API: https://stenly.id/api/v1
...
const createQRISPaymentGenspay = (orderId, amount, settings) => {
  return new Promise((resolve, reject) => {
    const baseUrl = (settings.genspay?.baseUrl || process.env.GENSPAY_BASE_URL || 'https://stenly.id/api/v1').trim();
    const apiKey = (settings.genspay?.apiKey || process.env.GENSPAY_API_KEY || '').trim();
    if (!apiKey) return reject(new Error('API Key belum dikonfigurasi'));

    // Stenly/GensPay membatasi order_id maksimal 25 karakter.
    orderId = String(orderId).slice(0, 25);
    let url;
    try { url = new URL(baseUrl.replace(/\/+$/, '') + '/charge'); } catch (e) { return reject(new Error('Base URL tidak valid')); }
    ...
    const req = https.request({
      ...
      res.on('end', () => {
        try {
          ...
          if (r.status !== 'success' || !qr) return reject(new Error(r.message || `Payment API error (HTTP ${res.statusCode})`));
          resolve({
            qr_string: qr,
            qr_image_url: d.qr_image_url || null,
            payment_url: d.payment_url || null,
            total_payment: Number(d.gross_amount || amount),
            expired_at: d.expires_at || null,
            gateway_status: d.status || 'pending'
          });
        } catch(e) { reject(new Error(`Gagal parse response payment API (HTTP ${res.statusCode})`)); }
      });
    });
    req.on('timeout', () => { req.destroy(); reject(new Error('Payment API timeout')); });
    req.on('error', e => reject(new Error('Network error: ' + e.message)));
    req.write(body); req.end();
  });
};
...
const DRIPSTORE_CATALOG_TIMEOUT_MS = 1200;
...
function _dsFindVariantExplicitStock(resp, variantId) {
  ...
  const walk = (node) => {
    ...
    if (id != null && String(id) === target) {
      ...
      // `null`/undefined means the provider did not expose physical stock.
      // Jangan Number(null) -> 0, karena itu membuat semua variant tanpa field
      // stock terlihat habis walaupun saldo provider masih cukup.
...
function getDripstoreVirtualStock(snapshot, variantId) {
  ...
  // Kapasitas beli = jumlah yang sanggup dibayar saldo. Tetapi kalau
  // products.php juga mengirim stok fisik/provider, stok provider tetap
  // menjadi batas atas. Jadi produk hanya dianggap tersedia kalau:
  //   provider stock > 0 DAN saldo cukup untuk >= 1 unit.
  const balanceCapacity = Math.max(0, Math.floor(balanceCents / costCents));
  const explicitStock = _dsFindVariantExplicitStock(snapshot.products, variantId);
  if (explicitStock !== null) return Math.min(balanceCapacity, explicitStock);
...
app.get('/', async (req, res) => {
  ...
  settings.contact = settings.contact && typeof settings.contact === 'object' ? settings.contact : {};
  ...
  if ((homeDsMode === 'live' || homeDsMode === 'hybrid') && settings.dripstore?.apiToken) {
    ...
    // JANGAN menahan first paint menunggu supplier. Cold-start provider bisa
    // lambat; render langsung memakai cache (kalau ada), lalu refresh di
    // background. Endpoint /api/catalog/stock akan memperbarui kartu setelah
    // halaman tampil.
  ...
  // Jangan buang product dari HTML ketika snapshot DripStore belum tersedia.
  // Pada cold-start cache provider memang masih null, sehingga filter stok di
  // server akan membuat seluruh katalog terlihat kosong. Card tetap dirender,
  // tombol Beli tetap disabled/Cek stok, lalu /api/catalog/stock mengaktifkan
  // hanya variant/product yang benar-benar punya stok provider + saldo cukup.
...
app.get('/buy/:id', async (req, res, next) => {
  try {
    res.set('Cache-Control', 'no-store, max-age=0');
    const products = await readFresh('products.json');
    const productList = Array.isArray(products) ? products : [];
    let product = productList.find(p => String(p.id) === String(req.params.id));
  ...
  // Pakai res.locals.settings yang sudah di-fetch oleh middleware (readFresh fallback)
  // Buat snapshot settings yang benar-benar aman untuk halaman /buy.
  // Jangan memutasi res.locals.settings karena object itu dipakai middleware/view lain.
  const rawSettings = res.locals.settings && typeof res.locals.settings === 'object'
    ? res.locals.settings
    : (readDB('settings.json') || {});
  const settings = {
    ...rawSettings,
    contact: rawSettings.contact && typeof rawSettings.contact === 'object' ? rawSettings.contact : {},
    dripstore: rawSettings.dripstore && typeof rawSettings.dripstore === 'object' ? rawSettings.dripstore : {},
    categoryLabels: rawSettings.categoryLabels && typeof rawSettings.categoryLabels === 'object' ? rawSettings.categoryLabels : {},
    siteName: rawSettings.siteName == null ? 'AGHA NL' : String(rawSettings.siteName),
    siteUrl: rawSettings.siteUrl == null ? '' : String(rawSettings.siteUrl)
  };
  const user = res.locals.user || getSessionUser(req);
  ...
  // Normalisasi produk harus selalu punya fallback. Satu record produk lama
  // yang korup tidak boleh membuat seluruh halaman /buy menjadi 500.
  try {
    product = normalizeProductBuyOptions(product);
  } catch (normalizeError) {
    console.error('[BUY PRODUCT NORMALIZE ERROR]', req.params.id, normalizeError?.stack || normalizeError);
    product = { ...product, pricingOptions: [], items: [] };
  }

  // Data lama/hasil edit admin kadang menyimpan field opsional sebagai
  // non-string/non-array. Normalisasi di backend supaya EJS tidak meledak
  // menjadi 500 hanya karena satu produk punya data legacy yang aneh.
  product.name = String(product.name || 'Produk');
  product.description = product.description == null ? '' : String(product.description);
  product.videoUrl = product.videoUrl == null ? '' : String(product.videoUrl);
  product.channelUrl = product.channelUrl == null ? '' : String(product.channelUrl);
  product.compatibility = product.compatibility == null ? '' : String(product.compatibility);
  product.image = product.image == null ? '' : String(product.image);
  product.badge = product.badge == null ? '' : String(product.badge);
  product.version = product.version == null ? '1.0' : String(product.version);
  product.category = product.category == null ? '' : String(product.category);
  product.platforms = Array.isArray(product.platforms) ? product.platforms.map(v => String(v)) : [];
  product.categories = Array.isArray(product.categories) ? product.categories.map(v => String(v)) : (product.category ? [product.category] : []);
  product.featureList = Array.isArray(product.featureList) ? product.featureList.map(v => String(v)) : (product.featureList ? [String(product.featureList)] : []);
  product.sold = Number.isFinite(Number(product.sold)) ? Number(product.sold) : 0;
  product.fakeSold = product.fakeSold != null && Number.isFinite(Number(product.fakeSold)) ? Number(product.fakeSold) : null;
  product.pricingOptions = Array.isArray(product.pricingOptions) ? product.pricingOptions.map(o => ({
    ...o,
    price: Number.isFinite(Number(o?.price)) ? Number(o.price) : 0,
    reseller_price: o?.reseller_price != null && Number.isFinite(Number(o.reseller_price)) ? Number(o.reseller_price) : null,
    strike_price: o?.strike_price != null && Number.isFinite(Number(o.strike_price)) ? Number(o.strike_price) : null
  })) : [];
  ...
  if ((buyDsMode === 'live' || buyDsMode === 'hybrid') && settings.dripstore?.apiToken) {
    ...
    // Jangan blokir halaman beli saat cold-start provider. Kalau cache belum
    // ada, render cepat dan refresh stok lewat endpoint setelah paint.
    warmDripstoreCatalog(settings);
  }
  let buyStockSummary;
  try {
    buyStockSummary = buildProductStockSummary(product, settings, buyProviderSnapshot);
  } catch (stockError) {
    // Stok/provider adalah data pendukung halaman. Kalau parser provider atau
    // data stok legacy bermasalah, halaman produk tetap harus bisa dirender.
    console.error('[BUY PRODUCT STOCK ERROR]', req.params.id, stockError?.stack || stockError);
    const fallbackItems = Array.isArray(product.items) ? product.items : [];
    const fallbackStockByOption = fallbackItems.map((item, index) => ({
      index,
      days: Number(item?.durationValue || 0),
      unit: item?.durationUnit === 'h' ? 'h' : 'd',
      stock: 0, localStock: 0, providerStock: 0, providerKnown: false,
      providerBacked: false, variantId: null
    }));
    buyStockSummary = {
      product, stockByOption: fallbackStockByOption, stockCount: 0,
      providerStockUnknown: false
    };
  }
  ...
  const transactionsRaw = readDB('transactions.json');
  ...
  // Semua field yang langsung disentuh EJS dibuat deterministic di sini.
  // Ini mencegah satu data produk legacy/korup mematikan seluruh route /buy.
  productSafe.id = String(productSafe.id || req.params.id);
  productSafe.items = Array.isArray(productSafe.items) ? productSafe.items : [];
  productSafe.pricingOptions = Array.isArray(productSafe.pricingOptions) ? productSafe.pricingOptions : [];
  productSafe.description = typeof productSafe.description === 'string' ? productSafe.description : '';
  productSafe.image = typeof productSafe.image === 'string' ? productSafe.image : '';
  productSafe.videoUrl = typeof productSafe.videoUrl === 'string' ? productSafe.videoUrl : '';
  productSafe.channelUrl = typeof productSafe.channelUrl === 'string' ? productSafe.channelUrl : '';
  productSafe.featureList = Array.isArray(productSafe.featureList) ? productSafe.featureList : [];
  productSafe.categories = Array.isArray(productSafe.categories) ? productSafe.categories : [];
  productSafe.platforms = Array.isArray(productSafe.platforms) ? productSafe.platforms : [];
  const hasLiveProvider = productSafe.items.some(i => i.providerBacked);
  ...
  const renderSiteUrl = String(res.locals.siteUrl || settings.siteUrl || `${req.protocol}://${req.get('host')}`).replace(/\/$/, '');
  res.render('pages/buy', { product: productSafe, settings, user, isReseller, hasPurchased, categoryLabels, siteUrl: renderSiteUrl });
  } catch (error) {
    console.error('[BUY PRODUCT ERROR]', req.params.id, error?.stack || error);
    return next(error);
  }
});
...
app.get('/api/catalog/stock', async (req, res) => {
  ...
  try {
    ...
    if ((mode === 'live' || mode === 'hybrid') && settings.dripstore?.apiToken) {
      ...
      if (!snapshot) {
        // Endpoint katalog tidak boleh membuat browser menunggu supplier.
        // Refresh provider tetap berjalan di background untuk request berikutnya.
        warmDripstoreCatalog(settings);
...
//
// 📖 Dokumentasi Integrasi: https://stenly.id/docs (Swagger API)
// Base URL API: https://stenly.id/api/v1
// ══════════════════════════════════════════════════════════════════
app.post('/webhook/genspay', async (req, res) => {
  try {
    ...
    // GensPay mendokumentasikan signature sebagai SHA-256(JSON body + API Key).
    // Gunakan raw body terlebih dahulu (paling aman bila provider menandatangani
    // byte JSON yang benar-benar dikirim), lalu dukung bentuk JSON.stringify(body)
    // yang juga dicontohkan di dokumentasi GensPay. Keduanya tetap membutuhkan
    // API Key rahasia, jadi fallback ini tidak melemahkan autentikasi.
    const rawSignature = crypto.createHash('sha256')
      .update(req.rawBody + apiKey)
      .digest('hex');
    const normalizedSignature = crypto.createHash('sha256')
      .update(JSON.stringify(req.body || {}) + apiKey)
      .digest('hex');

    // Perbandingan tahan timing-attack. Jangan gunakan ==/=== untuk signature.
    const supplied = Buffer.from(String(signatureHeader).trim().toLowerCase());
    const matches = [rawSignature, normalizedSignature].some(expected => {
      const candidate = Buffer.from(expected);
      return supplied.length === candidate.length && crypto.timingSafeEqual(supplied, candidate);
    });
    if (!matches) {
...
app.post('/admin/settings/genspay', requireAdmin, async (req, res) => {
  try {
    ...
    settings.genspay = {
      ...
      baseUrl: baseUrl !== undefined ? baseUrl : (settings.genspay?.baseUrl || 'https://stenly.id/api/v1')
    };
...
app.post('/admin/qris/test', requireAdmin, async (req, res) => {
  try {
    ...
    try {
      if (gateway === 'genspay') {
        const testSettings = { apiGateway: 'genspay', genspay: { apiKey, baseUrl: baseUrl || 'https://stenly.id/api/v1' } };
